...
version_string = get_version_from_jucer(pathlib.Path(root_path).joinpath("TICK.jucer"))
innosetup_line_to_patch = get_lines_for_string(pathlib.Path(root_path).joinpath("Installer/tick_installer.iss"), "#define MyAppVersion")
patch_packages(pathlib.Path(root_path).joinpath("Installer/TICK.pkgproj"), version_string)
print("patch inno script (Windows)")
patch_file(pathlib.Path(root_path).joinpath("Installer/tick_installer.iss"), innosetup_line_to_patch, f"#define MyAppVersion \"{version_string}\"")
